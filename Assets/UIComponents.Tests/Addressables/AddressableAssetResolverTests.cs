...
{
    [TestFixture]
    public class AddressableAssetSourceTests : AssetSourceTestSuite<AddressableAssetSource>
    {
        [UnityTest]
        ...
        {
            var source = new AddressableAssetSource();

            var existsPath = "Assets/UIComponents.Tests/Addressables/Assets/Component.uss";
            var existsTask = source.AssetExists(existsPath);

            yield return existsTask.AsEnumerator();

            Assert.That(source.AssetPathExistsCache[existsPath], Is.True);

            var notExistsPath = "Foobar";
            var notExistsTask = source.AssetExists(notExistsPath);

            yield return notExistsTask.AsEnumerator();

            Assert.That(source.AssetPathExistsCache[notExistsPath], Is.False);

            var existsTaskTwo = source.AssetExists(existsPath);
